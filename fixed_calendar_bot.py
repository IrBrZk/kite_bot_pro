#!/usr/bin/env python3
# fixed_calendar_bot.py - бот с исправленным календарем
import logging
...
calendar_service = CalendarService(booking_service)
...
async def start_booking(update, context):
    ...
    choose_date_text = language_manager.get_text(user_id, 'booking.choose_date')
...
async def handle_calendar_callback(update, context):
    ...
    if data.startswith('calendar_'):
        ...
        if calendar_service.is_date_available(date):
            ...
            choose_time_text = language_manager.get_text(user_id, 'booking.choose_time')
...
async def handle_booking_confirmation(update, context):
    ...
    if any(keyword in user_response.lower() for keyword in confirm_keywords):
        ...
        if booking:
            # Форматируем детали бронирования
...
def main():
    ...
    logger.info("🚀 Kite Bot Pro с исправленным календарем запускается...")
    ...
    logger.info("✅ Бот с исправленным календарем запущен и готов к работе!")
